...
public class CombinationSum {
    ...
    public int maxPathSum(TreeNode root) {
     ...
     private int calculateMaxPathSum(TreeNode node) {
        ...
        int left = Math.max(0, calculateMaxPathSum(node.left)); // Ignore negative paths
        int right = Math.max(0, calculateMaxPathSum(node.right)); // Ignore negative paths

        // Update the global maximum with the maximum path sum passing through the current node
        maxSum = Math.max(maxSum, left + right + node.val);

        // Return the maximum single-branch sum (left or right) to the parent
